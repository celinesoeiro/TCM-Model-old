...
SW_M = W_N['W_IE_ci_m']
SW_D = W_N['W_IE_ci_d']
SW_TR = W_N['W_II_ci_tr']
SW_TN = W_N['W_IE_ci_tn']

Ib = currents['I_CI_1'] + Idc*np.ones(n)
...
for t in range(1, len(time)):
    ...
    for k in range(1, n):        
        ...
        if (v_aux >= vp):
            ...
            v[k][t] = v_aux + dt*(neuron_contribution + self_feedback + layer_S + layer_M + layer_TR + layer_TN + layer_D + noise)
